<script type="text/javascript">
    ...
    let menu, animate, isHorizontalLayout = !1,
        SearchConfig = (document.getElementById("layout-menu") && (isHorizontalLayout = document.getElementById("layout-menu").classList.contains("menu-horizontal")), document.addEventListener("DOMContentLoaded", function () {
            ...
            0 < a.length && o && a[0].addEventListener("ps-scroll-y", function () {
            ...
            }), t = localStorage.getItem("templateCustomizer-" + templateName + "--Theme") || (window.templateCustomizer?.settings?.defaultStyle ?? document.documentElement.getAttribute("data-bs-theme")), window.Helpers.switchImage(t), window.Helpers.setTheme(window.Helpers.getPreferredTheme()), window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change", () => {
                ...
                window.Helpers.showActiveTheme(window.Helpers.getPreferredTheme()), s(), window.Helpers.initSidebarToggle(), document.querySelectorAll("[data-bs-theme-value]").forEach(a => {
                ...
                debug: !1,
                fallbackLng: "en",
                backend: {loadPath: assetsPath + "json/locales/.json"},
                returnObjects: !0
            }).then(function (e) {
                r()
            });
            ...
            let c = document.querySelectorAll(".dropdown-notifications-read"), m = (i && i.addEventListener("click", e => {
                c.forEach(e => {
            ...
            if (m && (window.innerWidth < window.Helpers.LAYOUT_BREAKPOINT ? window.Helpers.setNavbarFixed("fixed") : window.Helpers.setNavbarFixed("")), window.addEventListener("resize", function (e) {
                m && (window.innerWidth < window.Helpers.LAYOUT_BREAKPOINT ? window.Helpers.setNavbarFixed("fixed") : window.Helpers.setNavbarFixed(""), setTimeout(function () {
